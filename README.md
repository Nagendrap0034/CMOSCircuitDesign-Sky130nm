...
In CMOS circuit design, PMOS and NMOS transistors are connected together to form logic gates such as XOR, NAND, NOR, AND, OR, etc. These logic gates are the building blocks of all digital circuits.</br>

<img width="498" height="501" alt="image" src="https://github.com/user-attachments/assets/0f1df6c3-cd81-41d3-98c3-70f95b2b0062" />

The inverter shown above has certain electrical characteristics (Vin,Vout,Vdd,Vss). To understand its behavior, Perform SPICE simulations. These simulations helps analyze important parameters such as delay, switching behavior, and performance. Based on these results, determine the proper W/L ratio of the transistors.</br>
...
SPICE (Simulation Program with Integrated Circuit Emphasis) is a very important tool in circuit designing. It is used to analyze and predict the behavior of electronic circuits before actual fabrication.</br>

Modern design techniques such as clock tree synthesis, static timing analysis, and crosstalk evaluation are all based on SPICE simulations. Without SPICE, it won't be possible to calculate delays, and without delay information, physical design and timing verification would not be meaningful.</br>

For instance, consider a clock distribution network where buffers are connected with different capacitive loads. There are 2 levels of buffering.</br>

<img width="1206" height="392" alt="image" src="https://github.com/user-attachments/assets/a0dbe0ed-6981-4966-8d1d-7bf4d223ef61" />

After the SPICE simulation we obtain a "Delay Table", which includes input slew and output load. The intersection value of these two parameters is considered as Delay. Delay tables for both level 1 and level 2 buffers helps in timing analysis and optimization. This is calculated by circuit design and simulation.</br>
...
An NMOS transistor consist of P-type substrate, heavily doped with n+ region. There is an isolated region which isolates the transistor from other transistors. The n+regions are Source and Drain. There is an oxide layer above it, and top of it is metal deposition which is the Gate terminal.</br>

<img width="1195" height="507" alt="image" src="https://github.com/user-attachments/assets/fafb4e04-8d70-42dd-bae3-7570c38be028" />

**Threshold Voltage**
Threshold voltage(Vt) is an important parameter in MOSFET operation.</br>
Let us consider(Vgs=0), means source and drain terminal both are grounded. Body is also ground. P-substrate and n+ regions form PN junctions, behaving like diodes.There is no potential, high resistance, No channel is formed.</br>

<img width="1322" height="528" alt="image" src="https://github.com/user-attachments/assets/8e2fee4c-8616-4886-859c-d6c08d166541" />

Now apply a positive potential(Vgs>0). Gate is now positively charged and due to this there will be Accumulation of negative charges at the surface of substrate.</br>
...
Due to Accumulation of negative charges, there will be formation of Depletion Region, depleting of it's majority carriers i.e positive carriers here.</br>

<img width="831" height="402" alt="image" src="https://github.com/user-attachments/assets/335a3525-699e-4c66-8161-2201df2b9070" />

Now increase the Gate voltage further, there are positive charge carriers will be repelled and there will be increase of Depletion Width. On further increase of Gate voltage, will reach at a point where the surface gets inverted into an n-type material, this is called "Surface Inversion" or "Stronf Inversion". The value of Vgs voltage at which this inversion happens is called "Threshold Voltage".</br>

<img width="1332" height="551" alt="image" src="https://github.com/user-attachments/assets/03776359-fd17-4e04-9fd5-9212422217df" />

If Vgs is further increased --> There are no more negative charges that will be attracted towards the positive Vgs. The negative charges from n+ region will get attracted and so there will be a formation of channel between source and drain.</br>
...
Current can flow from Source to Drain. The channel has bridged the gap between source to drain regions. But as there is no Drain voltage so the elctrons will not move, this is "Cut Off Region" now.</br>

Let observe what happens when the potential of Body terminal is changed.</br>
...
When a positive voltage is applied to the substrate(Vsb>0),the depletion region increases further. Some charges from the channel are pulled towards the source.</br>

<img width="1247" height="617" alt="image" src="https://github.com/user-attachments/assets/65d128ea-03af-4dd4-af13-28b261e0fe66" />

Due to this the surface inversion will be slower. Therefore some extra potential has to be apllied to create inversion. The parameters such as Gamma comes from foundaries after simulation of which in SPICE we get the value for Vt (Threshold Voltage).</br>
...
Previously, we saw Cut Off region operation, now we will see the Resistive/Linear Region of operation by applying Drain-source voltage.
If Vgs is further increased, the channel width increases, more charge carriers are available for conduction.</br>

<img width="1305" height="508" alt="image" src="https://github.com/user-attachments/assets/29f349a8-c77e-4d13-a323-82b1fa6bfb74" />

The net Induced charge in the channel is propotional to (Vgs-Vt). Now apply very small Vds at start. And keep Vt=0.45V, Vgs is slightly greater than Vt.</br>

<img width="1317" height="567" alt="image" src="https://github.com/user-attachments/assets/aed1572f-fd2b-492d-82ca-bd1ce4a94b1e" />

Since the source is grounded and drain is at some potential, a voltage gradient accross the channel is created.</br>

<img width="1318" height="577" alt="image" src="https://github.com/user-attachments/assets/bf52a8c7-5553-4c74-888c-cdb43b09e3bc" />

Due to this, the Effective channel length is much lesser than the original channel length.</br>

<img width="797" height="510" alt="image" src="https://github.com/user-attachments/assets/c35a777b-b625-495c-bb37-9f92dfd92a64" />

Here, Y axis represents the width of transistor. X axis is the voltage along the channel.</br>
At each point, voltage is Vgs-V(x), which determines the current.</br>

<img width="817" height="558" alt="image" src="https://github.com/user-attachments/assets/a472661e-b9bd-4cfb-8437-ea5be2474fe2" />

### L2 Drift current theory
The effective channel voltage will vary with x, For instance at x=0, Vgs=1V and V(x)=0, So the Vgs-Vx=1V. At x=Vds=0.05V, Vgs-Vx=0.95V. This shows that induced charge depends on position.</br>
...
There are two types of currents; Drift and Diffusion current. Drift current dominates beacuse of the electric field.</br>

<img width="1285" height="618" alt="image" src="https://github.com/user-attachments/assets/e1ba1c63-b342-4377-9fe8-1a2e58c93e02" />

To calculate the drain current, consider the top view of transistor.</br>
...
As there is change of voltage across the channel length, this results in variation of carrier velocity. It depends on mobility and electric field.</br>

<img width="448" height="651" alt="image" src="https://github.com/user-attachments/assets/6518460a-1e4c-481f-8341-d99a1fa9a376" />

To dervie the current, integrate the above equation, where limits of dV will be from (0-Vds) and limits of dx will be from (0-L).</br>
...
Here: Cox, W/L ratio, Vgs, mobility and Vt are the technology parameters.</br>

<img width="387" height="211" alt="image" src="https://github.com/user-attachments/assets/ee457e12-6af7-4389-a11a-036ba5a4b652" />

Even though equation is quadratic in Vds, the device behaves as linear when (Vgs-Vt)>=Vds.</br>
...
To find the impact of Vgs and Vds on the drain current equation,consider different values of Vgs and Vds. If we consider different values of Vgs, under what condition the device will remain in Linear region depends on (Vgs-Vt) should be greater than Vds.</br>

<img width="591" height="172" alt="image" src="https://github.com/user-attachments/assets/d9735c9f-3a26-49aa-bc9b-62e64d65babe" />

Now the main question arises, How do we calculate Id for different values of 'Vgs' and at every value of 'Vgs'</br>
Sweep Vds till (Vgs-Vt) using linear equation for Id? SPICE simulation is the solution.</br>

### L5 Pinch-off region condition
There is also a Region of operation when Drain-source voltage exceeds beyond (Vgs-Vt), the region of operation is called "Saturation Region".
